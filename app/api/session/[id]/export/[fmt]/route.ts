...
export async function GET(
  req: Request,
  ctx: { params: Promise<{ id: string; fmt: string }> },
) {
  const { id, fmt: fmtParam } = await ctx.params;
  ...
  let leads: Lead[] = session.leads;
  if (scope === 'approved') leads = leads.filter(l => l.status === 'approved');
  else if (scope === 'crm_ready') leads = leads.filter(l => l.crm_ready);

  const rows = leads.map(l => {
    const o: Record<string, unknown> = {};
    for (const c of EXPORT_COLUMNS) o[c] = (l as unknown as Record<string, unknown>)[c] ?? '';
    return o;
  });
